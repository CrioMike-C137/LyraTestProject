...
void ULyraDamageExecution::Execute_Implementation(const FGameplayEffectCustomExecutionParameters& ExecutionParams, FGameplayEffectCustomExecutionOutput& OutExecutionOutput) const
{
	...
	if (DamageDone > 0.0f)
	{
		if (TargetTags->HasTag(LyraGameplayTags::Status_ManaShield))
		{
			const float CurrentMana{ ULyraManaComponent::FindManaComponent(HitActor)->GetMana() };
			float FinalDamage{ DamageDone };

			if (DamageDone > CurrentMana)
			{
				OutExecutionOutput.AddOutputModifier(FGameplayModifierEvaluatedData(ULyraHealthSet::GetDamageAttribute(), EGameplayModOp::Additive, DamageDone - CurrentMana));
				FinalDamage = CurrentMana;
			}

			OutExecutionOutput.AddOutputModifier(FGameplayModifierEvaluatedData(ULyraManaSet::GetManaDecreaseAttribute(), EGameplayModOp::Additive, FinalDamage));
		}
		else
		{
